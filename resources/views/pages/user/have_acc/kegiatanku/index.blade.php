    ...
    <div class="container my-5">
        <div class="nav-tabs-box">
            <ul class="nav nav-tabs justify-content-center border-0 mb-0">
                <li class="nav-item">
                    <a class="nav-link {{@$statusOpen['pendaftaran']}}" aria-selected="" data-bs-toggle="tab" href="#status">Status Pendaftaran</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link {{@$statusOpen['kegitan']}}" aria-selected="" data-bs-toggle="tab" href="#kegiatan">Kegiatan Aktif</a>
                </li>
            </ul>
        </div>
            ...
            <div class="tab-content mt-4">
                <div class="tab-pane fade {{@$statusOpen['pendaftaranselect']}}" id="status">
                    ...
                    <div class="container text-center py-5">
                        <img src="{{ asset('img/no-img.avif') }}" alt="No Activity" style="max-width: 300px;" class="mb-4">
                        <h2 class="text-secondary mb-3">Belum Ada Kegiatan</h2>
                        <p class="lead">Maaf, kamu belum memiliki kegiatan saat ini. Pastikan kamu sudah mengajukan posisi magang.</p>
                    ...
                    <div class="status-card">
                        ...
                        <div class="interview-content">
                            ...
                            <p class="interview-date">Tanggal Tes Kemampuan : {{date('d/m/Y', strtotime($value->tanggal_awal_tes_kemampuan))}} - {{ date('d/m/Y', strtotime($value->tanggal_akhir_tes_kemampuan)) }} </p>
                            @if($value->status_tes_kemampuan == 'diterima')
                            @elseif($value->status_tes_kemampuan == 'ditolak')
                            @else

                                @if(!empty($value->jawaban_tes_kemampuan))
                                    <div class="button-wrapper"> <button class="btn btn-danger link-wawancara" disabled>Link Tes Kemampuan</button></div>
                                @else
                                    <div class="button-wrapper"> <a href="{{ route('user.kegiatanku.soal') }}" class="btn btn-danger link-wawancara" >Link Tes Kemampuan</a></div>
                                @endif
                    ...
                    <div class="status-card">
                        ...
                        <div class="interview-content">
                            ...
                            <p class="interview-date">Tanggal Wawancara : {{date('d/m/Y', strtotime($value->tanggal_wawancara))}} {{$value->jam_wawancara}}</p>
                            <div class="button-wrapper">
                                <a href="{{$value->link_wawancara}}" target="_blank" class="btn btn-danger link-wawancara">Link Wawancara</a>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="status-card">
                        ...
                        <div class="interview-content">
                            <h5 class="fw-bold mb-1">{{ $value->posisi->nama ?? 'N/A' }}</h5>
                            <p class="mb-2">
                                Luar biasa! Kamu berhasil mendapatkan posisi sebagai {{ $value->posisi->nama ?? 'N/A' }}. Ayo mulai perjalanan hebat ini bersama kami!
                            </p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="container mt-4">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-8">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="overflow: visible;">
                                                        <div class="dropdown">
                                                            <button class="btn btn-sm btn-outline-secondary" type="button" id="dropdownAksi{{ $project->id }}" data-bs-toggle="dropdown" aria-expanded="false"
                                                                data-bs-offset="0,5" data-bs-auto-close="outside" data-bs-container="body"
                                                                data-bs-reference="parent">
                                                                <i class="fas fa-bars"></i>
                                                            </button>
                                                            <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownAksi{{ $project->id }}">
                                                                <li>
                                                                    <a class="dropdown-item btn-edit-judul" href="#"
                                                                        data-bs-toggle="modal" data-bs-target="#editLaporanModal"
                                                                        data-id="{{ $project->id }}" data-judul="{{ $project->title }}" data-type="{{ $project->jenis }}">
                                                                        <i class="fas fa-edit me-2 text-primary"></i>Edit
                                                                    </a>
                                                                </li>
                                                                <li>
                                                                    <a class="dropdown-item" href="{{ route('kegiatanku.project.detail', ['id' => $project->id]) }}">
                                                                        <i class="fas fa-info-circle me-2 text-info"></i>Detail Projek
                                                                    </a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                ...
                                @if($testimoni == true)
                                <div class=" mb-2">
                                    <div class="card p-3 text-center rounded" style="border: 1px solid #ddd; border-radius: 10px;  margin-top: 20px;">
                                        ...
                                        <button {{ $sertifikat == true ? 'disabled' : '' }} class="btn btn-danger rounded" style="border-radius: 10px;" data-bs-toggle="modal" data-bs-target="#testimoniModal">Testimoni</button>
                                    </div>
                                </div>
                                @endif
                                @if($sertifikat == true)
                                <div class="mb-2">
                                    <div class="card p-3 text-center rounded" style="border: 1px solid #ddd; border-radius: 10px;  margin-top: 10px;">
                                        ...
                                        @if($berisertifikat == true)
                                            <a href="{{ asset('file?page='.$ceksertifikat->location) }}" class="btn btn-danger rounded" style="border-radius: 10px;">Unduh</a>
                                        @else
                                            <button onClick="alertBeluBeri()" class="btn btn-danger rounded" style="border-radius: 10px;">Unduh</button>
                                        @endif
