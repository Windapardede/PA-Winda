...
@push('style')
    ...
    <style>
        ...
        .background-box {
            ...
            text-align: left;
            border-radius: 8px;
            /* pastikan ini tidak ada jika kamu tidak mau centering */
            display: block;
        }
    </style>
@endpush
...
@section('content')
            ...
            <div class="card-body">
                @if (isset($soal))
                    <div class="form-group">
                        ...
                        <div class="ps-3 text-start bg-light border rounded p-3" id="deskripsi">
                    ...
                    <div class="form-group mt-3">
                        ...
                        <div class="ps-3 text-start bg-light border rounded p-3" id="tanggal_pelaksanaan">
                            {{ date('d/m/Y', strtotime($soal->tanggal_mulai)) }} s/d
                    ...
                    <div class="form-group mt-3">
                        ...
                        <div class="ps-3 text-start bg-light border rounded p-3" id="soal_text">
                            {!! nl2br(e($soal->soal ?? 'Soal tidak tersedia.')) !!}
                        </div>
                    </div>
                    ...
                    <form id="uploadForm" action="{{ route('user.kegiatanku.upload_jawaban') }}" method="POST"
                        ...
                        <div class="form-group">
                            ...
                            @enderror
                            <small class="form-text text-muted">Unggah jawaban Anda dalam format <strong>PDF</strong> dengan
                                ukuran maksimal 5MB.</small>
                            @if (isset($kemampuan->jawaban_tes_kemampuan))
                                <small>File saat ini: <a
                                        href="{{ asset('file?page=' . $kemampuan->jawaban_tes_kemampuan) ?? asset('#') }}"
                                        target="_blank">{{ $kemampuan->jawaban_tes_kemampuan }}</a></small>
                            @endif
                        </div>
    ...
    <div class="modal fade modern-modal" id="konfirmasiKirimModal" tabindex="-1"
        aria-labelledby="konfirmasiKirimModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" form="uploadForm" name="submit_type" value="kirim" class="btn btn-confirm">Ya,
            ...
            submitButton.addEventListener('click', function(event) {
                if (!fileInput.files[0]) {
                    if (customFileUpload) {
                        ...
                        if (!invalidFeedback || !invalidFeedback.classList.contains(
                                'invalid-feedback')) {
                            invalidFeedback = document.createElement('div');
                            invalidFeedback.classList.add('invalid-feedback');
                            customFileUpload.parentNode.insertBefore(invalidFeedback, customFileUpload
                                .nextSibling);
                        }
                        invalidFeedback.textContent =
                            'Anda harus mengunggah file jawaban terlebih dahulu.';
                        invalidFeedback.style.display = 'block';
                    }
                    return;
                }
