...
@section('content')
    ...
    <section class="container-p">
        ...
        @foreach ($user as $posisi)
            {{-- Menggunakan $user karena di controller index() Anda menggunakan compact('user') --}}
                ...
                <div class="card-body">
                    ...
                    <div class="btn-wrapper">
                        ...
                        <a href="#" class="lihat-link" data-bs-toggle="modal" data-bs-target="#detailPosisiModal"
                            data-nama="{{ $posisi->nama }}" data-kuota="{{ $posisi->kuota_tersedia }}"
                            data-deskripsi="{{ $posisi->deskripsi }}" data-persyaratan="{{ $posisi->persyaratan }}"
            ...
            detailPosisiModal.addEventListener('show.bs.modal', function(event) {
                ...
                if (tahapan) {
                    ...
                    tahapanArray.forEach((item, index) => {
                        ...
                        stageItem.innerHTML =
                            `<div class="number">${index + 1}</div><div class="text">${item.trim()}</div>`;
