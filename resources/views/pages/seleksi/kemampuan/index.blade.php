                                ...
                                <tr id="row-{{ $item->id }}">
                                    ...
                                    <td>
                                        ...
                                        @if ($item->tanggal_awal_tes_kemampuan && $item->tanggal_akhir_tes_kemampuan)
                                            {{ date('d M Y', strtotime($item->tanggal_awal_tes_kemampuan)) }} -
                                            {{ date('d M Y', strtotime($item->tanggal_akhir_tes_kemampuan)) }}
                                        @else
                                            -
                                        @endif
                                    </td>
                                    <td>
                                        ...
                                        @if ($item->soal == 'ada')
                                            <a href="{{ route('kemampuan.show', $item->id) }}" class="eye-button"
                                    ...
                                    <td>
                                        ...
                                        @if (!empty($item->jawaban_tes_kemampuan))
                                            <a class="eye-button"
                                                ...
                                                target="_blank" aria-label="Lihat Jawaban">
                                                <i class="fa fa-eye"></i>
                                            </a>
                                        @else
                                            -
                                    ...
                                    <td class="text-center">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right"
                                                ...
                                                @php
                                                    $isFinalStatus =
                                                        $item->status_tes_kemampuan == 'diterima' ||
                                                        $item->status_tes_kemampuan == 'ditolak';
                                                ...
                                                {{-- Unggah Soal --}}
                                                <a href="{{ route('kemampuan.edit', $item->id) }}"
                                                    class="dropdown-item {{ $disableUnggahSoal ? 'disabled-option' : '' }}"
                                                ...
                                                {{-- Terima --}}
                                                <button type="button"
                                                    class="dropdown-item terima-btn {{ $disableTerimaTolak ? 'disabled-option' : '' }}"
                                                    data-toggle="modal" data-target="#terimaModal"
                                                    data-id="{{ $item->id }}"
                                                ...
                                                {{-- Tolak --}}
                                                <button type="button"
                                                    class="dropdown-item tolak-btn {{ $disableTerimaTolak ? 'disabled-option' : '' }}"
                                                    data-toggle="modal" data-target="#tolakModal"
                                                    data-id="{{ $item->id }}"
                                                    {{ $disableTerimaTolak ? 'disabled' : '' }}>
            ...
            <div class="modal-content modal-content-custom">
                ...
                <div class="modal-body modal-message-custom">
                    ...
                    <div class="form-group mt-3">
                        ...
                        <textarea class="form-control" id="catatanDitolakInput" rows="3" placeholder="Masukkan alasan penolakan..."
                            required></textarea>
                    </div>
                </div>
                <div class="modal-footer modal-buttons-custom">
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            if ($row.length) {
                                ...
                                $statusCell.empty().append('<span class="badge-status ' +
                                    statusBadgeClass + '">' + statusBadgeText + '</span>');
                                ...
                                $row.find('.dropdown-menu .dropdown-item').each(function() {
                                    ...
                                    if ($(this).is('a')) {
                                        $(this).attr('tabindex', '-1').attr('aria-disabled',
                                            'true');
                                    }
                                });
                                ...
                                Swal.fire('Informasi!',
                                    'Baris tabel tidak ditemukan, halaman akan dimuat ulang untuk pembaruan.',
                                    'info');
                                location.reload();
                            }
                        } else {
                            Swal.fire('Gagal!', response.message ||
                                'Terjadi kesalahan saat memperbarui status.', 'error');
                        }
                    },
                    error: function(xhr, status, error) {
                        Swal.fire('Error!', 'Terjadi kesalahan saat menghubungi server: ' + xhr
                            .responseText, 'error');
