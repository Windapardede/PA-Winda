...
class ProfileAdminController extends Controller
{
     ...
     * Digunakan jika Anda ingin menampilkan profil admin berdasarkan ID tertentu.
     *
     * @param  int  $id
    ...
    public function show($id)
    {
        ...
        $user = User::findOrFail($id); // Gunakan findOrFail agar langsung 404 jika tidak ditemukan

        // Anda bisa menambahkan logika otorisasi di sini,
        // misalnya hanya superadmin yang bisa melihat profil admin lain,
        // atau pastikan ID yang diminta adalah ID user yang sedang login.
        // if ($user->id !== Auth::id()) {
    ...
    public function update(Request $request) // Mengganti nama method dari 'updateProfile' ke 'update'
    {
        ...
        $user->position = $request->input('posisi');
        $user->email = $request->input('email');

        if ($request->hasFile('profile_photo')) {
            if ($user->image && $user->image !== 'images/profile_placeholder.jpg') {
        ...
        return response()->json([
            ...
            'profile_photo_url' => $user->image ? Storage::url($user->image) : asset('images/profile_placeholder.jpg')
        ]);
    }
    ...
    public function showChangePasswordForm()
    {
        ...
        return view('pages.profile.sandi.index');
    }
